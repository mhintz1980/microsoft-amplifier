...
def convert_skill_file(file_path: Path) -> bool:
    ...
    try:
        ...
        content = re.sub(
            r'from \.\.skills_framework\.skill_template import SkillLevel.*?\n',
            '',
            content
        )
        ...
        content = re.sub(r'    @property\s+def description\(self\) -> str:.*?(?=\n    def|\n\n|\Z)', '', content, flags=re.DOTALL)
        content = re.sub(r'    @property\s+def tags\(self\) -> list\[str\]:.*?(?=\n    def|\n\n|\Z)', '', content, flags=re.DOTALL)
        ...
        execute_pattern = r'    def execute\(self, context: SkillContext, level: SkillLevel = SkillLevel\.SUMMARY\) -> SkillResult:'
        content = re.sub(execute_pattern, '    async def execute(self, input_data: Any, context: SkillContext = None) -> SkillResult:', content)
    ...
    '''
        ...
        if 'def get_capabilities(self' not in content:
            capabilities_text = ', '.join(capabilities) if 'capabilities' in locals() else f"{class_name.replace('Skill', '').lower()} expertise"
