...
class DocToSkillConverter:
    ...
    def extract_content(self, soup, url):
        ...
        for h in main.find_all(["h1", "h2", "h3", "h4", "h5", "h6"]):
            ...
            if text:
                page["headings"].append({"level": h.name, "text": text, "id": str(h.get("id", ""))})
    ...
    def scrape_all(self):
        ...
        if self.dry_run:
            print("Mode: Preview only (no actual scraping)\n")
        else:
    ...
    def smart_categorize(self, pages):
        ...
        categories = {cat: [] for cat in category_defs}
        categories["other"] = []
...
def main():
    ...
    if args.enhance:
        print(f"\n{'=' * 60}")
        print("ENHANCING SKILL.MD WITH CLAUDE API")
        print(f"{'=' * 60}\n")
    ...
    if args.enhance_local:
        print(f"\n{'=' * 60}")
        print("ENHANCING SKILL.MD WITH CLAUDE CODE (LOCAL)")
        print(f"{'=' * 60}\n")
