...
class ConfigSplitter:
    ...
    def get_split_strategy(self) -> str:
        ...
        if self.strategy == "auto":
            ...
            if max_pages < 5000:
                ...
                return "none"
            if max_pages < 10000 and "categories" in self.config:
                print(f"ℹ️  Medium documentation ({max_pages} pages) - category split recommended")
                return "category"
            if "categories" in self.config and len(self.config["categories"]) >= 3:
    ...
    def create_router_config(self, sub_configs: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        router_config = {
            ...
            "_routing_keywords": {cfg["name"]: list(cfg.get("categories", {})) for cfg in sub_configs},
        }
